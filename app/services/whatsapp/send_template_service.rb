module Whatsapp
    class SendTemplateService
      attr_reader :phone_number_id, :version, :to, :template
  
      def initialize(phone_number_id:, version:, to:, template:)
        @phone_number_id = phone_number_id
        @version = version || 'v20.0'
        @to = to
        @template = template
        Rails.logger.info "📋 [SendTemplateService] Initialized with: phone_number_id=#{phone_number_id}, version=#{version}, to=#{to}, template=#{template}"
      end
      ...
      def perform
        ...
        request["Authorization"] = "Bearer #{ENV['SYSTEM_USER_ACCESS_TOKEN']}"
