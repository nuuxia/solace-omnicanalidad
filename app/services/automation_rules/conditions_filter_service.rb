...
class AutomationRules::ConditionsFilterService < FilterService
  ...
  def initialize(rule, conversation = nil, options = {})
    ...
    @csat_filters = @filters['csat'] || { 'rating' => { 'attribute_type' => 'standard', 'data_type' => 'integer' } }
  ...
  def apply_filter(query_hash, current_index)
    ...
    csat_filter = @csat_filters[query_hash['attribute_key']]
    ...
    when 'csat_response_created'
      ...
      if conversation_filter
        ...
        raise "Automation condition #{query_hash['attribute_key']} not supported."
      end
    end
  end
  ...
  def csat_query_string(current_filter, query_hash, current_index)
    ...
    case current_filter['attribute_type']
    when 'standard'
      " csat_survey_responses.rating #{filter_operator_value} #{query_operator} " if attribute_key == 'rating'
    end
  end
  ...
  def base_relation
    records = Conversation.where(id: @conversation.id)
                          ...
                          .joins('LEFT OUTER JOIN csat_survey_responses on csat_survey_responses.conversation_id = conversations.id')
